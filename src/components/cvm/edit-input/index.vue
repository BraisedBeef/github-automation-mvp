<template>
  <div>
    <template v-if="!show">
      <div>
        <span :style="textStyle" class="value mr4">{{ modelValue }}</span>
        <SvgIcon v-if="showEditIcon" :name="editIcon" style="cursor: pointer" @click="clickEdit" />
      </div>
    </template>
    <!-- 修改账号昵称 -->
    <template v-else>
...
const props = withDefaults(defineProps<PropType>(), {
  ...
  maxLength: 30,
  inputStyle: '',
  isTable: false, // 是否在表格中（样式修改）
  showEditIcon: true, // 显示icon
})
