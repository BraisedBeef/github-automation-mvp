<template>
  <div class="wrap">
    ...
    <div class="title-wrap df">
      <span class="title mr20">{{ ORDER_PRDDUCT_MAP[detail.productId] }}</span>
      <p class="df ac">
    ...
    <div class="product-wrap">
      ...
      <div class="df ac jsb amount-row pd0-20">
        ...
        <p class="rt">
          ...
          ><span class="text-warning ml20 font-bold">{{ detail.amount ? '-' : '' }}${{ detail.amount }} USD</span>
        </p>
      </div>
      <el-table
        ref="tableRef"
      ...
      >
        ...
        <el-table-column :label="t('payment.duration')">
          <template #default="{ row }">
            <span class="table-text">{{ t('payment.durationNum', [row.duration]) }}</span>
          </template>
        </el-table-column>
        <!-- 订单金额 -->
        <el-table-column align="right">
          ...
          <template #default="{ row }">
            <span class="text-warning mr20">{{ row.amount ? '-' : '' }}${{ row.amount }} USD</span>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <div class="payment-wrap df jfe ac page-box">
      <div class="df ac">
        <div class="mr20 df ac">
          <div class="text-14">
            {{ t('payment.refundPrice') }}
          </div>
          <el-text type="warning" class="text-24"> ${{ detail.amount }} USD </el-text>
        </div>
        <el-button :loading="loading" type="primary" @click="emits('buy')">
          {{ t('payment.comfirmSubmit') }}
        </el-button>
      </div>
    </div>
  </div>
...
import { ORDER_PRDDUCT_MAP } from './config'
import { BusinessType } from '@/views/cost-center/hooks/use-product-order'
