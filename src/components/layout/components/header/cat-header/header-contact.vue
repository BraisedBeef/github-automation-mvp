<template>
  <div class="header-contact">
    <el-dropdown trigger="hover" popper-class="contact-dropdown" placement="bottom-end" :teleported="true">
      <div class="drop">
        <span class="contact">{{ t('common.contact') }}</span>
        <SvgIcon name="cat-header-service" class="header-icon e" />
      </div>
      <template #dropdown>
          ...
          <div class="qr_code">
            ...
            <div class="qr_code_container">
              <div v-for="item in telegramList" :key="item.name" class="qr_code_box">
                <img :src="item.qrCodeUrl" class="qr_code_img" lt="" />
                <p class="telegram" @click="toWebsite(item.url)">
                  <span>{{ item.name }}</span> <SvgIcon name="telegram" class="header-correct" />
                </p>
              </div>
            </div>
          </div>
...
const telegramList = [
  {
    name: '@ahoneco',
    url: 'https://t.me/ahoneco',
    qrCodeUrl: imageContactQrCode1,
  },
  {
    name: '@zhouran99',
    url: 'https://t.me/zhouran99',
    qrCodeUrl: imageContactQrCode2,
  },
]
// const telegram = '@ahoneco'
// const telegramUrl = 'https://t.me/ahoneco'
const toWebsite = telegramUrl => {
