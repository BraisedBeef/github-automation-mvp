...
import { showBucketCreateNotification } from './components/create-bucket-notification'
...
const tableRef = ref<InstanceType<typeof TableV2> | null>(null)
...
const createBucketFn = async () => {
  ...
  if (!res.data.allowed) {
    showBucketCreateNotification({
      type: 'failed',
      message: `${t('accountSystem.createFail')}，${t('oss.maxBucketCount')}`,
    })
    return
  }
