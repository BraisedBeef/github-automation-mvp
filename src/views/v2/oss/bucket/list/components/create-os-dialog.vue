...
import { showBucketCreateNotification } from './create-bucket-notification'
...
const createBucket = async () => {
  ...
  loading.value = false
  if (data.code !== 200) {
    showBucketCreateNotification({
      type: 'failed',
      message: `${t('accountSystem.createFail')}，${t(data.message || data.msg)}`,
    })
  } else {
    showBucketCreateNotification({
      type: 'success',
...
:global(.el-notification.oss-bucket-create-notification) {
  min-width: 360px;
  max-width: 520px;
  padding: 0;
  overflow: hidden;
...
:global(.el-notification.oss-bucket-create-notification .el-notification__group) {
  display: flex;
  align-items: center;
  width: 100%;
  min-height: 56px;
...
:global(.el-notification.oss-bucket-create-notification .el-notification__title) {
  display: none;
}

:global(.el-notification.oss-bucket-create-notification .el-notification__content) {
  width: 100%;
  margin-top: 0;
}

:global(.el-notification.oss-bucket-create-notification .el-notification__closeBtn) {
  top: 50%;
  right: 16px;
  color: inherit;
...
:global(.el-notification.oss-bucket-create-notification--success) {
  color: #19a871;
  background: linear-gradient(0deg, rgb(34 176 125 / 8%), rgb(34 176 125 / 8%)), #fff;
  border-left: 3px solid #22b07d;
}

:global(.el-notification.oss-bucket-create-notification--failed) {
  color: #ef5a44;
  background: linear-gradient(0deg, rgb(240 78 58 / 8%), rgb(240 78 58 / 8%)), #fff;
  border-left: 3px solid #f04e3a;
}

:global(.oss-bucket-create-notification__body) {
  display: flex;
  gap: 10px;
  align-items: center;
  min-height: 20px;
...
:global(.oss-bucket-create-notification__icon) {
  flex-shrink: 0;
  width: 18px;
  height: 18px;
  color: currentcolor;
...
:global(.oss-bucket-create-notification__text) {
