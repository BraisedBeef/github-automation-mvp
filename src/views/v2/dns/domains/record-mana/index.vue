    ...
    >
      ...
      >
        <template #header>
          <div class="custom-table-header">
            ...
            >
              <template #content>
                <div class="x-tip">
                  <div class="x-tip__title">
                    {{ t('domainSet2.v2XOptimizeTipTitle') }}
                  </div>
                  <div class="x-tip__text">
                    <p>{{ t('domainSet2.v2XOptimizeTipDesc') }}</p>
                    <p>{{ t('domainSet2.v2XOptimizeTipStrategy') }}</p>
                    <p>{{ t('domainSet2.v2XOptimizeTipExample') }}</p>
                    <p>{{ t('domainSet2.v2XOptimizeTipExampleIp') }}</p>
                    <p>{{ t('domainSet2.v2XOptimizeTipExampleSelect') }}</p>
                    <p>{{ t('domainSet2.v2XOptimizeTipExampleExclude') }}</p>
                    <p class="x-tip__note">
                      {{ t('domainSet2.v2XOptimizeTipNote') }}
                    </p>
                  </div>
                </div>
              </template>
        ...
        <template #default="{ row, $index }">
          ...
          <!-- M优化 -->
          <div v-else-if="item.prop === 'xOptimize'" v-show="row.type !== 'edit'" class="table-x-optimize">
            <span :class="row.xOptimize ? 'text-success' : 'record-mana-muted'">
          ...
          <div v-else-if="item.prop === 'status'" class="table-status">
            <template v-if="row.status !== void 0 && row.type !== 'edit'">
              <span class="table-status__dot" :class="getActive(row.status, analysisStatus)?.class" />
              <span class="table-status__text" :class="getActive(row.status, analysisStatus)?.class">
...
import { _s_setItem, setSessionItemRaw } from '@/utils/storage'
...
:deep(.record-mana-table) {
  ...
  th.el-table__cell:nth-child(2) .cell,
  td.el-table__cell:nth-child(2) .cell {
    width: 0 !important;
    min-width: 0 !important;
    padding: 0 !important;
    overflow: hidden;
...
.table-status {
  ...
  .table-status__dot.text-success {
    background-color: var(--success-color);
  }

  .table-status__dot.text-warning {
    background-color: var(--warning-color);
  }

  .table-status__dot.text-error {
