<template>
  <div class="cdn-domain-detail-cache-v2">
    <section class="cdn-domain-detail-cache-v2__section">
      <div class="cdn-domain-detail-cache-v2__section-head">
        <h3 class="cdn-domain-detail-cache-v2__title">
          {{ t('cacheRule') }}
        </h3>
        <p class="cdn-domain-detail-cache-v2__desc">
          {{ t('cacheRuleTip') }}
        </p>
      </div>

      <div class="cdn-domain-detail-cache-v2__action-row">
        <el-button :disabled="isReadOnly" type="primary" @click="cacheDialogHandle">
          {{ t('edit') }}
        </el-button>
      </div>

      <TableV2 :data="cache.cache_rule_conf.rules" class="cdn-domain-detail-cache-v2__table" table-layout="auto">
        <el-table-column prop="type" :label="t('type')" min-width="120">
          <template #default="{ row }">
            {{ getValueMap(cacheType, row.type) }}
          </template>
        </el-table-column>
        <el-table-column prop="content" :label="t('content1')" min-width="180">
          <template #default="{ row }">
            {{ row.content || '-' }}
          </template>
        </el-table-column>
        <el-table-column prop="priority" :label="t('priority')" min-width="100" />
        <el-table-column prop="url_params_status" :label="t('URLparams')" min-width="140">
          <template #default="{ row }">
            {{ getValueMap(urlParams, row.url_params_status) }}
          </template>
        </el-table-column>
        <el-table-column prop="url_params" :label="t('URLvalue')" min-width="180">
          <template #default="{ row }">
            {{ row.url_params || '-' }}
          </template>
        </el-table-column>
        <el-table-column prop="cache_status" :label="t('cacheFollowOrigin')" min-width="140">
          <template #default="{ row }">
            {{ getValueMap(cacheMode, row.cache_status) }}
          </template>
        </el-table-column>
        <el-table-column prop="ttl" :label="t('cacheExpireTime')" min-width="140">
          <template #default="{ row }">
    ...
    <section class="cdn-domain-detail-cache-v2__section">
      ...
      <div class="cdn-domain-detail-cache-v2__action-row">
        <el-button :disabled="isReadOnly" type="primary" @click="browserDialogHandle">
          {{ t('edit') }}
        </el-button>
      </div>

      <TableV2 :data="cache.browser_cache_conf.rules" class="cdn-domain-detail-cache-v2__table" table-layout="auto">
        <el-table-column prop="type" :label="t('type')" min-width="160">
          <template #default="{ row }">
            {{ getValueMap(cacheType, row.type) }}
          </template>
        </el-table-column>
        <el-table-column prop="content" :label="t('content1')" min-width="220">
          <template #default="{ row }">
            {{ row.content || '-' }}
          </template>
        </el-table-column>
        ...
        <el-table-column prop="ttl" :label="t('cacheExpireTime')" min-width="160">
          <template #default="{ row }">
            {{ row.cache_status == 2 ? row.ttl + getValueMap(cacheTime, row.ttl_unit) : '-' }}
          </template>
        </el-table-column>
    ...
    <section class="cdn-domain-detail-cache-v2__section">
      ...
      <div class="cdn-domain-detail-cache-v2__action-row">
        <el-button :disabled="isReadOnly" type="primary" @click="statusCodeHandle('add')">
          {{ t('add') }}
        </el-button>
      </div>

      <TableV2 :data="cache.http_status_cache_conf.rules" class="cdn-domain-detail-cache-v2__table" table-layout="auto">
        <el-table-column prop="http_status" :label="t('statusCode')" min-width="220" />
        <el-table-column prop="ttl" :label="t('cacheTime')" min-width="220">
          <template #default="{ row }">
            <div>{{ row.ttl + getValueMap(cacheTime, row.ttl_unit) }}</div>
          </template>
        </el-table-column>
        <el-table-column :label="t('operation')" min-width="220">
          <template #default="{ row }">
            <div class="table-handle">
              <el-link :disabled="isReadOnly" type="primary" @click="statusCodeHandle('edit', row)">
                {{ t('edit') }}
              </el-link>
              <el-link
                :disabled="isReadOnly"
    ...
    <section class="cdn-domain-detail-cache-v2__section">
      ...
      <div class="cdn-domain-detail-cache-v2__action-row">
        <el-button :disabled="isReadOnly" type="primary" @click="editURLrewrite(URLrewrite.rawForm)">
          {{ t('add') }}
        </el-button>
      </div>

      <TableV2 :data="URLrewrite.tableData" class="cdn-domain-detail-cache-v2__table" table-layout="auto">
        <el-table-column :label="t('matchType')" prop="mate_type" min-width="180">
          <template #default="{ row }">
            {{ row.mate_type ? t('allPathMatch') : t('preMatch') }}
          </template>
        </el-table-column>
        <el-table-column :label="t('waitRewriteURL')" prop="source_url" min-width="260" />
        <el-table-column :label="t('targetPath')" prop="target_url" min-width="260" />
        <el-table-column :label="t('priority')" prop="priority" min-width="120" />
        <el-table-column :label="t('operation')" min-width="140">
          <template #default="{ row, $index }">
            <div class="table-handle">
              <el-link :disabled="isReadOnly" type="primary" @click="editURLrewrite(row, $index)">
                {{ t('edit') }}
              </el-link>
              <el-link :disabled="isReadOnly" type="primary" @click="updateURLrewrite(true, $index)">
                {{ t('delete') }}
              </el-link>
            </div>
          </template>
...
const isTrueTime = row => {
  if (row.cache_status != 2) return true
...
const statusCodeCurrent = computed(() => {
  return statusCode.map((vv: any) => {
    vv.disabled = cache.value.http_status_cache_conf.rules.some(v => v.http_status == vv.value)
    return vv
  })
})
...
const addStatusCodeRule = async () => {
  ...
  if (statusCodeDialog.title === 'add') {
    ...
    await _addStatusCodeRule({ id: route.query.id, form: { rule } }).then(() => {
      getCacheConfig()
      notification({ type: 'success', msg: 'setSuccess' })
    })
  } else {
    await _updateStatusCodeRule({ id: route.query.id, form: { rule } }).then(() => {
      getCacheConfig()
      notification({ type: 'success', msg: 'setSuccess' })
    })
  }
...
.cdn-domain-detail-cache-v2 {
  display: flex;
  flex-direction: column;
  gap: 16px;
}

.cdn-domain-detail-cache-v2__section {
  padding: 20px;
  background: #fff;
}

.cdn-domain-detail-cache-v2__section-head {
  display: flex;
  flex-direction: column;
  gap: 8px;
}

.cdn-domain-detail-cache-v2__title {
  margin: 0;
  font-size: 16px;
  font-weight: 500;
  line-height: 24px;
...
.cdn-domain-detail-cache-v2__desc {
  margin: 0;
  font-size: 14px;
  line-height: 22px;
  color: #828b9c;
...
.cdn-domain-detail-cache-v2__action-row {
  margin: 16px 0;
}

.cdn-domain-detail-cache-v2__table {
  margin-top: 16px;
}
