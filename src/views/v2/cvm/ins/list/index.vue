<template>
  <div class="cvm-ins-list-v2">
    <PageBreadcrumbToolbar :items="[t('ins.list.ins')]" @doc="openHelpDoc">
      <template #title-extra>
        <div class="cvm-ins-list-v2__area">
          <AreaSelect v-model="area" @change-area="changeArea" />
        </div>
      </template>
    </PageBreadcrumbToolbar>

    <InfoNotice v-if="noticeVisible" class="cvm-ins-list-v2__notice">
      <div class="cvm-ins-list-v2__notice-content">
        <span>实例列表支持按状态、可用区和计费模式筛选，批量操作会根据实例状态自动启用。</span>
        <button class="cvm-ins-list-v2__notice-close" type="button" @click="noticeVisible = false">
          <SvgIcon name="close" />
        </button>
      </div>
    </InfoNotice>

    <section class="cvm-ins-list-v2__panel">
      <div class="cvm-ins-list-v2__toolbar">
        <div class="cvm-ins-list-v2__actions">
          <el-button v-if="checkPermission(permMap.ins.CREATE_VM)" type="primary" @click="create">
            {{ t('ins.list.create') }}
          </el-button>
          <TooltipBtn
        ...
        <div class="cvm-ins-list-v2__tools">
          <FilterInput
            ref="filterInputRef"
            class="cvm-ins-list-v2__filter-input"
            :placeholder="t('ins.list.listPh')"
            :filter-list="iptFilterConfig"
            @search="searchFn"
          />
          <CdnIconButton class="cvm-ins-list-v2__icon-btn" @click="changeShow(exportRef)">
            <SvgIcon name="cat-cdn-download" />
          </CdnIconButton>
          <CdnIconButton class="cvm-ins-list-v2__icon-btn" @click="refresh">
      ...
      <TableV2
        ...
        @change="handleV2PaginationChange"
      >
        <el-table-column type="selection" width="54" />

        <el-table-column min-width="190" :label="t('ins.list.idName')" show-overflow-tooltip>
          <template #default="{ row }">
            <div class="cvm-ins-list-v2__name-cell">
              <el-badge
                v-if="row.autoRenew"
                :value="t('disk.list.continue')"
                class="cvm-ins-list-v2__badge"
                type="warning"
              >
                <div class="cvm-ins-list-v2__link">
                  <Copy :text="row.insUuid" style="display: inline-block" @handle-click="toDetail(row)" />
                </div>
              </el-badge>
              <div v-else class="cvm-ins-list-v2__link">
                <Copy :text="row.insUuid" style="display: inline-block" @handle-click="toDetail(row)" />
              </div>
              <div class="cvm-ins-list-v2__subtext">
                {{ row.instanceName || '-' }}
              </div>
            </div>
          </template>
        ...
        <el-table-column min-width="120" :label="t('ins.list.monitor')">
          <template #default="{ row }">
            <button class="cvm-ins-list-v2__action-link" type="button" @click="toMonitor(row)">
        ...
        <el-table-column min-width="130">
          ...
          <template #default="{ row }">
            <div class="cvm-ins-list-v2__status-cell">
              <StatusGroup :list="INS_STATE_ARR_MAP" :value="row.state" />
              <SvgIcon v-if="row.loadingState" class="cvm-ins-list-v2__status-loading" name="loading" />
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column min-width="280" :label="t('ins.list.config')" show-overflow-tooltip>
          <template #default="{ row }">
            <div class="cvm-ins-list-v2__config-cell">
              <div class="cvm-ins-list-v2__config-main">
                {{ row.specificationInfo?.vcpus }}vCPUs | {{ row.specificationInfo?.ram }}GiB |
        ...
        <el-table-column min-width="220" :label="t('ins.list.IPv4')" show-overflow-tooltip>
          <template #default="{ row }">
            <div class="cvm-ins-list-v2__ip-cell">
              <div v-if="handleRowIp(row).show && handleRowIp(row).mainPublicIp" class="cvm-ins-list-v2__ip-line">
                <TableCopy
                  :show-text="`${handleRowIp(row).mainPublicIp}${t('ins.list.eip')}`"
                  :text="handleRowIp(row).mainPublicIp"
                />
              </div>
              <div v-if="handleRowIp(row).show && handleRowIp(row).mainPrivateIp" class="cvm-ins-list-v2__ip-line">
                <TableCopy
        ...
        <el-table-column fixed="right" :min-width="optionsWidth" :label="t('ins.list.opts')">
          <template #default="{ row }">
            <div class="cvm-ins-list-v2__ops">
              <Select
...
import UnBindIP from '@/views/cvm/ins/list/components/un-bind-ip/index.vue'
import UnBindNetworkCard from '@/views/cvm/ins/list/components/un-bind-network-card/index.vue'
...
import BindIP from '@/views/cvm/ins/list/components/bind-ip/index.vue'
...
import Export from '@/views/cvm/ins/list/components/export/index.vue'
import Expans from '@/views/cvm/ins/list/components/expans/index.vue'
import ChangeSystem from '@/views/cvm/ins/list/components/change-system/index.vue'
import FlowToTime from '@/views/cvm/ins/list/components/flow-to-time/index.vue'
import TimeToFlow from '@/views/cvm/ins/list/components/time-to-flow/index.vue'
...
import { documentUrl, httpUrl } from '@/config/base-config'
...
const handleV2PaginationChange = (pagination: { page: number; pageSize: number }) => {
  paginationConfig.pagination.page = pagination.page
  paginationConfig.pagination.pageSize = pagination.pageSize
  listInstanceFn()
}
...
.cvm-ins-list-v2__area {
  display: flex;
  align-items: center;
  min-width: 240px;
}

:deep(.cvm-ins-list-v2__area .select-area) {
...
.cvm-ins-list-v2__notice {
  :deep(.v2-info-notice__text) {
    width: 100%;
  }
}

.cvm-ins-list-v2__notice-content {
  display: flex;
  gap: 16px;
  align-items: center;
...
.cvm-ins-list-v2__panel {
  padding: 24px 28px;
  background: #fff;
  border: 1px solid #e8ecf3;
}

.cvm-ins-list-v2__toolbar {
  display: flex;
  gap: 16px;
...
.cvm-ins-list-v2__filter-input {
  width: 330px;
}

.cvm-ins-list-v2__icon-btn {
  flex-shrink: 0;
}

.cvm-ins-list-v2__more {
  margin-left: 8px;
}

.cvm-ins-list-v2__name-cell,
.cvm-ins-list-v2__config-cell,
.cvm-ins-list-v2__ip-cell {
  display: flex;
  flex-direction: column;
  gap: 4px;
}

.cvm-ins-list-v2__link,
.cvm-ins-list-v2__action-link {
  color: #06f;
}
...
.cvm-ins-list-v2__subtext {
  color: #4e5969;
}

.cvm-ins-list-v2__status-cell {
  display: flex;
  gap: 6px;
  align-items: center;
}

.cvm-ins-list-v2__status-loading {
  width: 14px;
  height: 14px;
}

.cvm-ins-list-v2__config-main,
.cvm-ins-list-v2__ip-line {
  color: #191c23;
}

.cvm-ins-list-v2__ops {
  display: flex;
  gap: 16px;
  align-items: center;
}
