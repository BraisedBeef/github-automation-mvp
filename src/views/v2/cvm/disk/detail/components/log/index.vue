<template>
  <section class="cvm-disk-log-v2">
    <div class="cvm-disk-log-v2__toolbar">
      <BtnsGroup v-model="btnValue" :list="btnsGroupList" :style="{ width: '100px' }" @change-value="changeDate" />

      <el-date-picker
        v-model="dateTime"
        class="cvm-disk-log-v2__picker"
        type="daterange"
        start-placeholder=""
        end-placeholder=""
        :shortcuts="shortcuts"
    ...
    <TableV2
      ...
      :empty-min-height="320"
      show-pagination
      @change="paginationChange"
    >
      <el-table-column min-width="220">
        <template #header>
          <div class="cvm-disk-log-v2__th">
            <span>{{ t('disk.log.optTime') }}</span>
            <el-tooltip :content="t('common.time_zone_desc')" placement="top" effect="light">
              <SvgIcon name="cat-cdn-help" class="cvm-disk-log-v2__th-icon" />
            </el-tooltip>
          </div>
        </template>
      ...
      <el-table-column min-width="220" :label="t('disk.log.optName')">
        <template #default="{ row }">
          {{ row.operateName || '-' }}
        </template>
      </el-table-column>

      <el-table-column min-width="220" :label="t('disk.log.opts')">
        <template #default="{ row }">
          {{ row.operate || '-' }}
        </template>
      </el-table-column>
      ...
      <el-table-column min-width="160" :label="t('disk.log.optRes')">
        <template #default="{ row }">
          <span v-if="row.state == 1" class="cvm-disk-log-v2__result cvm-disk-log-v2__result--primary">
            {{ LOG_STATE_MAP[row.state] }}
          </span>
          <span v-else-if="row.state == 2" class="cvm-disk-log-v2__result cvm-disk-log-v2__result--success">
            {{ LOG_STATE_MAP[row.state] }}
          </span>
          <span v-else-if="row.state == 3" class="cvm-disk-log-v2__result cvm-disk-log-v2__result--error">
            {{ LOG_STATE_MAP[row.state] }}
          </span>
          <span v-else>-</span>
        </template>
...
const btnsGroupList = [
  { label: t('disk.log.3d'), value: '3d' },
  { label: t('disk.log.7d'), value: '7d' },
]

const shortcuts = [
  { text: t('disk.log.3dLast'), value: () => getDateTime(3, 'day') },
  { text: t('order.oneWeek'), value: () => getDateTime(1, 'week') },
...
const changeDate = v => {
  dateTime.value = v.value === '3d' ? shortcuts[0].value() : shortcuts[1].value()
}
...
.cvm-disk-log-v2__th-icon {
  color: #8b95a7;
}

.cvm-disk-log-v2__result {
  font-weight: 500;
}

.cvm-disk-log-v2__result--primary {
  color: #06f;
}

.cvm-disk-log-v2__result--success {
  color: #23b26d;
}

.cvm-disk-log-v2__result--error {
  color: #f04438;
}
