  ...
  <div class="access">
    <div class="page-box">
      ...
      <div class="mt20 df ac">
        ...
        />
        <el-link
          v-show="access_config.referer_conf.status"
          :disabled="+route.query.status > 2 || route.query.is_lock == 1"
          type="primary"
          @click="editReferer"
        >
      ...
      <div v-show="!access_config.referer_conf.status" class="mt20">
        {{ t('noAntiTheftRule') }}
      </div>
      <div v-show="access_config.referer_conf.status" class="mt20">
        referer{{ access_config.referer_conf.type ? t('whiteList') : t('blackList') }}
        {{ access_config.referer_conf.allow_empty ? t('allow') : t('refuse') }}referer
      </div>
      <el-input
        v-show="access_config.referer_conf.status"
    ...
    <div class="page-box mt20">
      <div class="head">IP {{ t('blackAndWhitelistConfig') }}</div>
      <div class="tip mt20">
      ...
      <div class="mt20 df ac">
        ...
        />
        <el-link
          v-show="access_config.ip_filter_conf.status"
          :disabled="+route.query.status > 2 || route.query.is_lock == 1"
          type="primary"
          @click="editIP"
        >
      ...
      <div v-show="access_config.ip_filter_conf.status">
        <div class="df mt20">
          <div class="label">
            {{ t('type') }}
          </div>
          <div>IP{{ access_config.ip_filter_conf.rule.ip_type ? t('whiteList') : t('blackList') }}</div>
        </div>
        <div class="df mt20">
          <div class="label">
            {{ t('rule') }}
    ...
    <div class="page-box mt20">
      ...
      <div class="mt20 df ac">
        ...
        />
        <el-link
          v-show="access_config.ip_frequency_conf.status"
          :disabled="+route.query.status > 2 || route.query.is_lock == 1"
          type="primary"
          @click="openIPfreq"
        >
      ...
      <div v-show="access_config.ip_frequency_conf.status" class="mt20 df">
        <div class="label">
          {{ t('configStatus') }}
        </div>
        <div>{{ access_config.ip_frequency_conf.freq }}QPS</div>
      </div>
    </div>
    ...
    <div class="page-box mt20">
      ...
      <div class="mt20">
        <el-button type="primary" :disabled="+route.query.status > 2 || route.query.is_lock == 1" @click="openAuthCalc">
          {{ t('authCalc') }}
        </el-button>
      </div>
      <div class="mt20 df ac">
        ...
        />
        <el-link
          v-show="access_config.auth_conf.status"
          :disabled="+route.query.status > 2 || route.query.is_lock == 1"
          type="primary"
          @click="editAuth"
        >
      ...
      <div v-show="access_config.auth_conf.status" class="df mt20">
        <el-form label-width="auto" label-position="left" style="flex: 1">
          <el-form-item :label="t('authMode')">
          ...
          <el-form-item :label="t('key1')">
            {{ access_config.auth_conf.auth_key || '--' }}
          </el-form-item>
          <el-form-item :label="t('key2')">
            {{ access_config.auth_conf.auth_key_backup || '--' }}
          </el-form-item>
          <el-form-item :label="t('signParams')">
            {{ access_config.auth_conf.auth_parameter || '--' }}
          </el-form-item>
        </el-form>
        <el-form label-width="auto" label-position="left" class="ml20" style="flex: 1">
          <el-form-item :label="t('trueTime')">
            {{ access_config.auth_conf.valid_time || '--' }}
          </el-form-item>
          <el-form-item :label="t('timeMethod')">
    ...
    >
      ...
      >
        <el-form-item :label="t('originServerType')">
          <!-- <CatSelect v-model="auth_calc.form!.auth_type" :list="authTypeList" /> -->
          <CatRadioGroup v-model="auth_calc.form!.auth_type" :list="authTypeList" />
        </el-form-item>
        <el-form-item :label="t('authAgol')">
          <!-- <CatSelect v-model="auth_calc.form!.encrypt_manager" :list="authAlgorithmList" /> -->
        ...
        >
          <div class="df fdc w-full">
            <!-- <TipInput :rule="KEY_REG(auth_calc.form!.auth_key)"
              errorText='keyErrorTip' /> -->
    ...
    <div class="page-box mt20">
      ...
      <div class="mt20 df ac">
        ...
        />
        <el-link
          v-show="access_config.user_agent_conf.status"
          :disabled="+route.query.status > 2 || route.query.is_lock == 1"
          type="primary"
          @click="editAgent"
        >
      ...
      <div v-show="access_config.user_agent_conf.status">
        <div class="df mt20">
          <div class="label">
            {{ t('type') }}
          </div>
          <div>User-Agent {{ access_config.user_agent_conf.rule.type ? t('whiteList') : t('blackList') }}</div>
        </div>
        <div class="df mt20">
          <div class="label">
            {{ t('rule') }}
    ...
    <div class="page-box mt20 role">
      ...
      <div class="mt20 df ac">
        ...
        />
        <el-link
          v-show="access_config.remote_auth_conf.status"
          :disabled="+route.query.status > 2 || route.query.is_lock == 1"
          type="primary"
          @click="editRemote"
        >
      ...
      <div v-show="access_config.remote_auth_conf.status" class="df ac mt10">
        <div class="tip">*{{ t('remoteAuthTip') }}</div>
      </div>
      <div v-show="access_config.remote_auth_conf.status" class="df mt20">
        <el-form label-position="left" label-width="auto" style="flex: 1">
          <el-form-item :label="t('remoteAuthAddress')">
          ...
          <el-form-item :label="t('authFileType')">
            {{ getValueMap(fileTypeList, access_config.remote_auth_conf.file_type) }}
          </el-form-item>
        </el-form>
        <el-form label-position="left" label-width="auto" class="ml20" style="flex: 1">
          <el-form-item :label="t('authTimeoutMin')">
...
const getAccessConfig = async () => {
  await _getAccessConfig(route.query.id as any).then(res => {
    ...
    for (let key in access_config.value) {
...
const cancelAuthCalc = () => {
  for (let key in auth_calc_result.value) {
