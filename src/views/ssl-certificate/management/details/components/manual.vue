<template>
  <div class="tpl-container">
    <p class="tips">
      {{ t('ssl-certificate.management.details.448561-0') }}{{ verify_type_map[factory_id || 1]
      }}{{ t('ssl-certificate.management.details.448561-1') }}
    </p>
    <el-steps class="steps" direction="vertical">
      <el-step class="step-item">
        ...
        <template #title>
          <p class="step-title">
            {{ t('ssl-certificate.management.details.448561-2') }}
          </p>
        </template>
        <template #description>
          <p class="step-description">
            {{ t('ssl-certificate.management.details.448561-3') }}
          </p>
        </template>
      </el-step>
      <el-step class="step-item">
        ...
        <template #title>
          <p class="step-title">
            {{ t('ssl-certificate.management.details.448561-4') }}
          </p>
        </template>
        <template #description>
          <p class="step-description">
            {{ t('ssl-certificate.management.details.448561-5') }}
          </p>
          <el-card v-loading="validateInfoStore.loading" class="mt20" body-style="background: #F9FAFC;" shadow="never">
            ...
            >
              <div class="info-card">
                ...
                >
                  <CatKVList :gutter="20">
                    <CatKVItem class="kvs" :label="t('ssl-certificate.management.details.448561-6')">
                      <template #label>
                        <p class="label">
                          {{ t('ssl-certificate.management.details.448561-6') }}
                        </p>
                      </template>
                    ...
                    <CatKVItem class="kvs" :label="t('ssl-certificate.management.details.448561-7')">
                      <template #label>
                        <p class="label">
                          {{ t('ssl-certificate.management.details.448561-7') }}
                        </p>
                      </template>
                      <p class="value auto-wrap">
                        {{ v_info.record_value }}
                    ...
                    <CatKVItem class="kvs" label="TTL">
                      ...
                      <p class="df ac value">600({{ t('ssl-certificate.management.details.448561-8') }})</p>
                    </CatKVItem>
                  </CatKVList>
                </div>
              </div>
              <TipsBar :tips="t('ssl-certificate.management.details.448561-9')" color="#f7b500" background="#fff3d3" />
            </template>
      ...
      <el-step class="step-item">
        ...
        <template #title>
          <p class="step-title">
            {{ t('ssl-certificate.management.details.448561-10') }}
          </p>
        </template>
        <template #description>
          <div class="df ac mb10">
            <el-link type="primary" style="margin-right: 10px" @click="handleHelp">
              {{ t('ssl-certificate.management.details.448561-11') }}
              <SvgIcon name="storage-link" class="icon" style="margin-left: 5px" />
            </el-link>
            <p style="font-size: 14px">
              {{ t('ssl-certificate.management.details.448561-12') }}
            </p>
          </div>
          <div class="df ac">
            <el-link href="/ssl/hosting" type="primary" style="margin-right: 10px">
              {{ t('ssl-certificate.management.details.448561-13') }}
              <SvgIcon name="storage-link" class="icon" style="margin-left: 5px" />
            </el-link>
            <p style="font-size: 14px">
              {{ t('ssl-certificate.management.details.448561-14') }}
            </p>
          </div>
        </template>
      </el-step>
    ...
    <el-form-item>
      <el-button type="default" @click="handleBack">
        {{ t('ssl-certificate.management.details.448561-15') }}
      </el-button>
    </el-form-item>
  </div>
  <el-dialog v-model="verifyStore.dialogVisible" title="" width="550" class="verify-type-dialog" center>
    <template #header>
      <p class="title">
        {{ t('ssl-certificate.management.details.448561-16') }}
      </p>
    </template>
    <el-select v-model="verifyStore.verifyType" placeholder="" style="width: 100%">
      <el-option v-for="item in certificateVerifyRadios" :key="item.value" :label="item.label" :value="item.value" />
    </el-select>
    <template #footer>
      <div class="dialog-footer">
        <el-button type="primary" :loading="verifyStore.loading" @click="changVerifyType">
          {{ t('ssl-certificate.management.details.448561-17') }}
        </el-button>
        <el-button @click="verifyStore.dialogVisible = false">
          {{ t('ssl-certificate.management.details.448561-18') }}
        </el-button>
      </div>
    </template>
  </el-dialog>
  <el-dialog v-model="viewErrorDialogVisible" title="" width="550" class="error-reason-dialog" center>
    <template #header>
      <p class="title">
        {{ t('ssl-certificate.management.details.448561-19') }}
      </p>
    </template>
    <p>{{ t('ssl-certificate.management.details.448561-20') }}</p>
    <br />
    <p>
      {{ t('ssl-certificate.management.details.448561-21') }}
      {{ t('ssl-certificate.management.details.448561-22') }}
    </p>
    <br />
    <p>
      {{ t('ssl-certificate.management.details.448561-23') }}
    </p>
    <br />
    <p>
      {{ t('ssl-certificate.management.details.448561-24') }}t('ssl-certificate.management.details.448561-25'){{
        t('ssl-certificate.management.details.448561-26')
      }}
    </p>
    <br />
    <p>{{ t('ssl-certificate.management.details.448561-27') }}</p>
    <template #footer>
      <div class="dialog-footer">
...
import { certificateVerifyRadios, SSL_CERTIFICATE_FACTORY_ID } from '@/views/ssl-certificate/management/config'
...
const { cert_id, verify_type } = route.query as { cert_id?: string; verify_type?: number }
...
const getValidateInfo = async () => {
  // 获取验证信息
  if (!cert_id) {
    message.error(t('ssl-certificate.management.details.448561-28'))
    return
  }
  try {
    ...
    if (code === 200 && data) {
      ...
      message.error(msg || t('ssl-certificate.management.details.448561-29'))
    }
  } finally {
...
const handleCopy = async (v: string) => {
  ...
  message.success(t('ssl-certificate.management.details.448561-30'))
}
  ...
  try {
    ...
    if (code === 200 && data) {
      ...
      if (data.fail_domain.length > 0) {
        ...
        message.success(t('ssl-certificate.management.details.448561-31'))
      }
    } else {
      verifyDomainStore.status = '2'
      message.error(msg || t('ssl-certificate.management.details.448561-32'))
    }
  } catch (e) {
    // verifyDomainStore.status = '2'
  } finally {
...
const changVerifyType = async () => {
  if (props.verifyType === verifyStore.verifyType - 1) {
  ...
  try {
    ...
    const { code, msg } = await _changeVerifyTypes({
      cert_id: cert_id,
      verify_types: verifyStore.verifyType,
    })
    if (code === 200) {
      // 验证域名
      ...
      message.error(msg || t('ssl-certificate.management.details.448561-33'))
    }
  } finally {
