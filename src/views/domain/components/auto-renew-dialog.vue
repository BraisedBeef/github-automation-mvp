<template>
  <CatDialog
    ...
    :title="props.auto_renew === 2 ? t('domain.components.361776-0') : t('domain.components.361776-1')"
    width="80%"
    destroy-on-close
    @confirm="renewHandle"
  >
    <CatCdnTable :data="renew.table" :show-overflow-tooltip="true">
      <el-table-column prop="domain" :label="t('domain.components.361776-2')" />
      <!-- 注册时间 -->
      <el-table-column prop="register_time" :label="t('domain.components.361776-3')">
        <template #header>
          <div class="df ac">
            {{ t('domain.components.361776-3') }}
      ...
      <!-- 到期时间 -->
      <el-table-column prop="expired" :label="t('domain.components.361776-4')">
        <template #header>
          <div class="df ac">
            {{ t('domain.components.361776-4') }}
            <el-tooltip :content="t('common.time_zone_desc')" placement="top" effect="light">
              <SvgIcon class="ml10" name="cat-cdn-help" />
            </el-tooltip>
          </div>
      ...
      <!-- 剩余年限 -->
      <el-table-column prop="left_days" :label="t('domain.components.361776-5')">
        <!-- <template #default="{ row }">{{ left_days||'--' }}</template> -->
      </el-table-column>
      <!-- 续费时长 -->
      <template v-if="props.auto_renew === 1">
        <el-table-column prop="renew_years" :label="t('domain.components.361776-6')">
          <template #default="{ row }">
            <span v-if="row.is_owner === 1">{{ renewList.find(v => v.value === props.year)?.label }}</span>
            <el-text v-else type="danger">
              {{ t('domain.components.361776-7') }}
            </el-text>
          </template>
        </el-table-column>
        <!-- 续费后到期时间 -->
        <el-table-column prop="expired_after_renew" :label="t('domain.components.361776-8')">
          <template #header>
            <div class="df ac">
              {{ t('domain.components.361776-8') }}
              <el-tooltip :content="t('common.time_zone_desc')" placement="top" effect="light">
                <SvgIcon class="ml10" name="cat-cdn-help" />
              </el-tooltip>
            </div>
        ...
        <el-table-column prop="amount" :label="t('domain.components.361776-9')">
          <template #default="{ row }">
      ...
      <el-table-column :label="t('domain.components.361776-10')" fixed="right">
        <template #default="{ $index }">
          <el-link :disabled="renew.table.length <= 1" type="primary" @click="renew.table.splice($index, 1)">
            {{ t('domain.components.361776-11') }}
          </el-link>
        </template>
      </el-table-column>
    </CatCdnTable>
...
import { _domainOwnership, _updateAutoRenew } from '@/apis/domain'
...
const renewList = reactive([{ label: t('domain.components.361776-12'), value: 1 }])
...
const getRenewList = async () => {
  ...
  if (code === 20000) {
    ...
    message.error(data.message || t('domain.components.361776-13'))
  }
  renew.loading = false
}
...
const renewHandle = async () => {
  if (!renew.table.every(v => v.is_owner === 1)) {
    message.error(t('domain.components.361776-14'))
    return
  }
  try {
    const { message, code } = await _updateAutoRenew({
    ...
    if (code === 20000) {
      renew.show = false
      message.success(props.auto_renew === 1 ? t('domain.components.361776-15') : t('domain.components.361776-16'))
      emit('success')
    } else {
      message.error(message || t('domain.components.361776-17'))
    }
  } catch (error) {
