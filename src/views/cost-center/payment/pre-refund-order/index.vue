  ...
  <div class="wrap">
    ...
    <div class="payment-wrap df jfe ac page-box">
      <div class="df ac">
        ...
        <el-button :loading="loading" type="primary" @click="refund">
...
const { loading, changeServerType } = useIns()
const cvmStore = useCvmStore()

const serialNo = ref<string>('') // 预退款单号
...
const refund = () => {
  ...
  if (product === 'ins') {
    // 调整配置
    if (operate === 'setconfig') changeServerType(true, cvmStore.timeCostReduceConfigOrder[serialNo.value], false)
  }

  if (!product || !operate) {
    // 退款失败
    message.warning(t('cvmCpm.refundFail'))
  }
}
